...
| :---: | :--- | :---: | :--- | :---: | :--- |
| 1 | **Trần Tuấn Tú** | `2A202602840` | Full-stack RAG Engineer / Trưởng nhóm | `main` | Phụ trách thu thập dữ liệu, indexing, hybrid retrieval, đánh giá và QA (Tasks 1–7, 13–14) |
| 2 | **Đào Duy Hiếu** | `2A202602651` | Full-stack Engineer | `main` | Phụ trách Golden Dataset và Evaluation Runner cho Tasks 11–12 |
| 3 | **Ngô Minh Thu** | `2A202603679` | Retrieval & Generation Engineer | `main` | Phụ trách fallback retrieval, hoàn thiện pipeline và generation có citation (Tasks 8–10) |
...
- **Các module trực tiếp triển khai:**
  ...
  4. **Evaluation Report (Task 13):** Hoàn thành báo cáo đánh giá chuyên sâu `RESULT.md`, phân tích nguyên nhân lỗi metadata chunk-0 và đề xuất giải pháp.
  5. **Chatbot Streamlit UI & Final QA (Task 14):** Nâng cấp giao diện người dùng `app.py` với tính năng phân tách nguồn trích dẫn trực tiếp và ngữ cảnh nền; đưa toàn bộ 26/26 tests về trạng thái xanh.
...
### **Ngô Minh Thu**
- **Mã học viên:** `2A202603679`
- **Vai trò:** Retrieval & Generation Engineer
- **Nhánh thực hiện:** `main`
- **Các module trực tiếp triển khai:**
  1. **Fallback & Pipeline hoàn chỉnh (Tasks 8–9):** Tích hợp PageIndex vectorless search và hiệu chuẩn ngưỡng cosine fallback `0.66` (in-domain $\ge 0.7172$, out-of-domain $\le 0.6044$).
  2. **Generation có Citation (Task 10):** Tích hợp LLM `gemini-3.5-flash-lite`, xử lý Lost-in-the-middle reordering, ràng buộc trích dẫn chunk ID chính xác và Safe Refusal khi ngoài domain.
